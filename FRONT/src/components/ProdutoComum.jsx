  ...
  useEffect(() => {
    async function buscaFoto(id_produto) {
      try {
        ...
        // MUDANÇA: Começa em 0 e vai até 2 (3 fotos no total)
        for (let posicao = 0; posicao < 3; posicao++) {
          let response;

          try {
            // MUDANÇA: Usa posicao 0, 1, 2
            response = await ky
          ...
          // MUDANÇA: Armazena na posição correta do array
          novasFotos[posicao] = response.data.url;
        }
              ...
              {tab === "composicao" && (
                <div className="font-secondary">
                  <p className="mt-6 text-[16px]">
                    ativo hidratante que carrega até mais de mil vezes o seu
                    peso em água, nutre e hidrata a pele, além de prevenir e
                    suavizar os sinais do tempo. nossa fórmula conta com oitos
                    formas e três pesos moleculares distintos desse ativo, o que
                    proporciona sua penetração em diferentes camadas da pele.
                  </p>
                  <p className="mt-12 text-[16px]">
                    também conhecido como Pro-Vitamina B5, o pantenol tem alto
                    poder hidratante devido sua capacidade de atrair e reter
                    umidade. além disso, promove ação calmante, suavizante e
                    anti-inflamatória.
                  </p>
                  <p className="mt-12 text-[16px]">
                    comum no hemisfério norte, esse ingrediente era usado desde
                    a China Imperial para manter a pele limpa e luminosa e
                    diminuir a aparência dos sinais do tempo. na nossa fórmula,
                    ele age na recuperação da luminosidade da pele, estimula a
                    firmeza e contribui para suavizar linhas finas.
                  </p>
                </div>
              )}
            </div>
