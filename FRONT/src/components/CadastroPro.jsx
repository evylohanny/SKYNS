...
function CadastroPro() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!peso.trim()) newErrors.peso = "Informe o peso.";
    ...
    try {
      const produtoData = {
        ...
        quantidade_estrelas: quantidadeEstrelas
          ? Number(quantidadeEstrelas)
          : 0,
        categoria: categoria,
        peso: Number(peso),
      ...
      if (!res.ok) {
        ...
        throw new Error(
          `Erro ao cadastrar produto: ${res.status} ${txt || ""}`
        );
      }
  ...
  return (
    <form
      ...
      className="w-[80%] min-h-screen bg-white rounded-sm shadow p-10 flex flex-col"
    >
      <div className="flex flex-row w-full gap-15">
        ...
        <div className="flex flex-col gap-4 w-[60%] font-secondary">
          ...
          <div className="flex flex-col gap-2 relative">
            <label className="font-semibold text-gray2/80">
              Categoria de peles
            </label>
            <select
              value={categoria}
            ...
            <img
              src={setaSelectCinza}
              alt="seta select"
              className="pointer-events-none absolute right-3 top-13 -translate-y-1/2 w-3 h-3 peer-focus:hidden"
            />
            <img
              src={setaSelectPurple}
              alt="seta select focus"
              className="hidden peer-focus:block pointer-events-none absolute right-3 top-13 -translate-y-1/2 w-3 h-3"
            />
            <div
              className={`text-purpledark w-4/6 h-4 text-sm transition-opacity duration-500 ${
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div
                className="flex items-center peer w-full border rounded-md px-3 py-2 text-sm text-gray2/80 outline-none border-gray3/50
          ...
          <div className="flex flex-col gap-2">
            <div className="flex justify-between items-center">
              ...
              <span className="font-semibold text-gray2/80">
                {completaDescricao.length} até 350
              </span>
            </div>
            <textarea
              maxLength={350}
          ...
          <hr className="border-gray2/50 mt-4 mb-4" />
          ...
          <div>
            <div className="grid grid-cols-2 gap-3 text-sm text-gray1">
              {componentesPermitidos.map((item, idx) => {
                const isDisabled =
                  !personalizado || !componentesPermitidos.includes(item);
          ...
          checked:bg-purpledark transition-colors duration-200
          disabled:checked:bg-gray-400 disabled:border-gray-300 disabled:bg-gray-200"
                    />
                    {item}
                  </label>
                );
          ...
          <div className="flex justify-end">
            <button
              type="submit"
              disabled={sending}
              className={`bg-purpledark hover:bg-blue hover:text-purpledark text-white px-6 py-2 rounded-md text-sm transition ${
